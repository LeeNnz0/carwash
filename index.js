...
document.addEventListener("DOMContentLoaded", function() {
  ...
  useOtherModel.addEventListener("change", () => {
    ...
    section.style.display = useOtherModel.checked ? "flex" : "none";
...
function filterCards() {
  ...
  noRecordsMessage.style.display = anyVisible ? "none" : "block";
}
function addExpense() {
...
function renderExpenses() {
  ...
  expenseArray.forEach((exp, i) => {
    ...
    li.innerHTML = `${exp.date} - ${exp.desc} <span class="badge bg-danger rounded-pill">₱${exp.amount}</span>`;
...
function exportToPDF() {
  ...
  recordList.querySelectorAll(".card").forEach(card => {
    if (card.dataset.paid === "true") {
      ...
      const plateMatch = text.match(/Plate:\s(.+?)\s\|/);
      ...
      const price = priceMatch?.[1] ? parseInt(priceMatch[1]) : 0;
      const service = serviceMatch?.[1]?.trim() || "";
      const boy = boyMatch?.[1]?.trim() || "";
      const date = dateMatch?.[1]?.trim() || new Date().toLocaleString();

      tableData.push([plate, model, boy, price, service, date]);
      pdfTotal += price;
    }
  });
  ...
  doc.autoTable({
    startY: 20,
    head: [["Plate", "Model", "Boy", "Price", "Service", "Date & Time"]],
    body: tableData,
    styles: { fontSize: 10 },
  });

  // Add totals (no peso sign)
  let nextY = doc.lastAutoTable.finalY + 10;
  doc.setFontSize(12);
  doc.text(`Total Amount (Paid Only): ${pdfTotal}`, 14, nextY);
  doc.text(`Total Expenses: ${totalExpenses}`, 14, nextY + 8);
  doc.text(`Net Total: ${pdfTotal - totalExpenses}`, 14, nextY + 16);

  // Expenses Breakdown (no peso sign)
  if (expenseArray.length) {
    doc.setFontSize(11);
    doc.text("Expenses Breakdown:", 14, nextY + 30);
    expenseArray.forEach((exp, i) => {
      const lineY = nextY + 38 + i * 7;
      if (lineY > 280) {
        doc.addPage();
        nextY = 10;
      }
      doc.text(`- ${exp.desc}: ${exp.amount}`, 14, lineY);
    });
  }
...
// Add input capitalization
function setupInputCapitalization() {
